...
# docker buildx build --tag blogdemo --file .pi/Dockerfile --platform linux/arm64 --cache-from blogdemo:cache --cache-to blogdemo:cache --load --progress tty .

# save the image to result-rootfs.tar
docker save --output ./.tmp/result-rootfs.tar blogdemo
...
sudo bash -c "echo blogdemo > ./.tmp/result-rootfs/etc/hostname"
...
# Make the image file
fallocate -l 4G "images/blogdemo.img"

# loop-mount the image file so it becomes a disk
sudo losetup --find --show images/blogdemo.img
